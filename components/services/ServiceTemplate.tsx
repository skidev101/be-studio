...
const ServiceTemplate = ({ config }: ServiceTemplateProps) => {
  ...
  return (
    <main className="min-h-screen">
      {/* HERO */}
      <section className={`relative overflow-hidden bg-linear-to-br ${colors.gradient}`}>
        <div className="absolute inset-0 bg-[url('data:image/svg+xml;base64,')] opacity-40"></div>
        
        <div className="relative mx-auto max-w-6xl px-6 py-20 md:py-28">
          <div className={`inline-flex items-center gap-2 rounded-full border ${colors.badge} px-4 py-2 text-sm font-medium backdrop-blur-sm`}>
            <Zap className="h-4 w-4" />
            {hero.badge}
          </div>
          
          <h1 className="mt-6 text-4xl md:text-6xl font-bold tracking-tight text-white leading-tight max-w-4xl">
            {hero.headline}
          </h1>
        ...
        <div className="mx-auto max-w-4xl px-4 md:px-6">
          ...
          <div className="mt-16 grid gap-6 md:grid-cols-3">
            {problem.painPoints.map((point) => (
              <div key={point.desc} className="rounded-2xl bg-slate-50 p-6 md:p-8 border border-slate-200">
                <div className={`text-3xl font-bold ${colors.accent}`}>{point.stat}</div>
        ...
        <div className="mx-auto max-w-6xl px-4 md:px-6">
          ...
          <div className="mt-16 grid gap-8 lg:grid-cols-3">
            {process.steps.map((step, idx) => (
              ...
              >
                <div className={`absolute -top-4 left-6 md:left-8 inline-flex h-12 w-12 items-center justify-center rounded-full ${colors.buttonPrimary} text-white font-bold text-lg shadow-lg`}>
                  {idx + 1}
                </div>
                ...
                {step.timeline && (
                  <div className={`mt-4 text-sm font-semibold ${colors.accent}`}>
                    {step.timeline}
                  </div>
                )}
              </div>
        ...
        <div className="mx-auto max-w-5xl px-4 md:px-6">
          ...
          <div className="mt-16 grid gap-4 md:grid-cols-2">
            {deliverables.items.map((item) => (
              <div
                key={item}
                className={`flex items-center md:items-start gap-2 md:gap-4 rounded-2xl border-2 border-slate-200 bg-slate-50 p-4 md:p-6 transition-all hover:border-${config.tier === 'premium' ? 'blue' : config.tier === 'standard' ? 'purple' : 'emerald'}-400`}
              >
                <CheckCircle2 className={`size-4 md:size-6 ${colors.accent} shrink-0 mt-1`} />
              ...
              {pricing.packages.map((pkg) => (
                <div
                  ...
                  className={`rounded-3xl bg-white p-8 shadow-lg ${
                    pkg.recommended ? `ring-4 ring-${config.tier === 'premium' ? 'blue' : 'purple'}-500 ring-offset-4` : 'border-2 border-slate-200'
                  }`}
                >
                  {pkg.recommended && (
                    <div className={`inline-block rounded-full ${colors.buttonPrimary} px-4 py-1 text-sm font-semibold text-white`}>
                      Recommended
                    </div>
                  )}
                  ...
                  <ul className="mt-8 space-y-4">
                    {pkg.features.map((feature) => (
                      <li key={feature} className="flex items-start gap-3">
                        <CheckCircle2 className={`h-5 w-5 ${colors.accent} shrink-0 mt-0.5`} />
                        <span className="text-slate-600">{feature}</span>
                      </li>
                    ))}
                  </ul>
                  <Button 
                    className={`mt-8 w-full rounded-full ${pkg.recommended ? colors.buttonPrimary : 'bg-slate-900 hover:bg-slate-800'} text-white`}
                  >
                    Choose {pkg.name}
                  </Button>
                </div>
